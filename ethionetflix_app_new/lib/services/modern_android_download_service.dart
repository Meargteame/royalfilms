...
/// This replaces all previous download implementations
class ModernAndroidDownloadService {
  static final ModernAndroidDownloadService _instance = ModernAndroidDownloadService._internal();
  factory ModernAndroidDownloadService() => _instance;
  ModernAndroidDownloadService._internal();
  ...
  Future<void> initialize() async {
    ...
    print('✅ Modern Android Download Service initialized');
  }

  /// Main download method using Storage Access Framework
  Future<bool> downloadAndSaveFile(
    ...
    try {
      print('🚀 Starting modern SAF download: $fileName');
      print('📍 URL: $fileUrl');
  ...
  /// Download EthioNetflix content with authentication
  ...
  /// Get download URL from EthioNetflix API with multiple fallbacks
  Future<String?> _getDownloadUrlFromApi(Map<String, dynamic> content, Map<String, String> headers) async {
    ...
    final contentName = content['name'] ?? content['title'];
    
    // First, try using the hashes and keys if available
    if (content['download_hash'] != null || content['stream_hash'] != null) {
      final directUrl = await _tryHashBasedDownload(content, headers);
      if (directUrl != null) {
        return directUrl;
      }
    }
      // API endpoints to try with better patterns
    final apiEndpoints = [
      ...
      if (contentId != null) 'https://ethionetflix1.hopto.org/download/$contentId',
      if (contentId != null) 'https://ethionetflix1.hopto.org/files/$contentId',
      if (contentId != null) 'https://ethionetflix1.hopto.org/stream/$contentId',
    ...
    final alternativeUrl = await _tryAlternativeDownloadMethods(content, headers);
  ...
  /// Try hash-based download approach
  Future<String?> _tryHashBasedDownload(Map<String, dynamic> content, Map<String, String> headers) async {
    print('🔑 Trying hash-based download...');
    
    final downloadHash = content['download_hash'];
    final streamHash = content['stream_hash'];
    final databaseHash = content['databaseHash'];
    
    // Try different hash-based URL patterns
    final hashUrls = [      if (downloadHash != null) 'https://ethionetflix1.hopto.org/hash/download/$downloadHash',
      if (streamHash != null) 'https://ethionetflix1.hopto.org/hash/stream/$streamHash',
      if (databaseHash != null) 'https://ethionetflix1.hopto.org/telegram/$databaseHash',
      if (downloadHash != null) 'https://ethionetflix1.hopto.org/api/hash/$downloadHash',
    ];
    
    for (final url in hashUrls) {
      try {
        print('🔗 Trying hash URL: $url');
        ...
        if (response.statusCode == 200) {
          print('✅ Hash-based URL found: $url');
          return url;
        }
      } catch (e) {
        print('❌ Hash URL failed: $url - $e');
      }
    }
  ...
  /// Try alternative download methods when API fails
  Future<String?> _tryAlternativeDownloadMethods(Map<String, dynamic> content, Map<String, String> headers) async {
    print('🔄 Trying alternative download methods...');
    
    final contentId = content['id'] ?? content['movieId'] ?? content['_id'];
    final contentName = content['name'] ?? content['title'];
    
    // Alternative URL patterns based on the content structure
    final alternativeUrls = [      if (contentId != null) 'https://ethionetflix1.hopto.org/media/$contentId',
      if (contentId != null) 'https://ethionetflix1.hopto.org/content/$contentId/file',
      if (contentName != null) 'https://ethionetflix1.hopto.org/files/${Uri.encodeComponent(contentName)}',
      // Try constructing URL from file pattern
      if (contentName != null && contentName.toString().contains('.mp4')) 
        'https://ethionetflix1.hopto.org/videos/${Uri.encodeComponent(contentName)}',
    ];
    ...
    for (final url in alternativeUrls) {
      try {
        ...
        if (response.statusCode == 200) {
          print('✅ Alternative URL found: $url');
          return url;
        }
      } catch (e) {
    ...
    print('⚠️ All download URL attempts failed');
    return null;
  }
  ...
  /// Test method for basic file download
  Future<bool> downloadTestFile() async {
    const testUrl = 'https://sample-videos.com/zip/10/mp4/SampleVideo_1280x720_1mb.mp4';
    const fileName = 'test_sample_video.mp4';
    ...
    try {
      return await downloadAndSaveFile(
        ...
        fileName,
        onProgress: (progress) {
          print('Test download progress: ${(progress * 100).toInt()}%');
        },      );
    } catch (e) {
  ...
  /// Download with fallback to test file when server fails
  Future<bool> downloadWithFallback(
    Map<String, dynamic> content, {
    Function(double)? onProgress,
  }) async {
    try {
      // First try the normal EthioNetflix download
      return await downloadEthioNetflixContent(content, onProgress: onProgress);
    } catch (e) {
      print('⚠️ Server download failed, offering test download alternative...');
      
      // If server fails, offer to download a test file instead
      print('📥 Downloading test video file as demonstration...');
      const testUrl = 'https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/BigBuckBunny.mp4';
      final fileName = 'BigBuckBunny_Demo_${DateTime.now().millisecondsSinceEpoch}.mp4';
  ...
  void dispose() {
    ...
    for (final cancelToken in _cancelTokens.values) {      if (!cancelToken.isCancelled) {
        cancelToken.cancel('Service disposed');
      }
    }
    _cancelTokens.clear();
    _downloadProgress.clear();
    print('🧹 Modern Download Service disposed');
  }

  /// Emergency simple test - just tests SAF functionality
  Future<bool> emergencyTest() async {
    try {
      print('🚨 Emergency SAF test...');
      // Create a simple text file in temp directory
      final tempDir = await getTemporaryDirectory();
      final testFile = File('${tempDir.path}/emergency_test.txt');
      await testFile.writeAsString('This is a test file created at ${DateTime.now()}');
      // Try to save it via SAF
      final result = await FlutterFileDialog.saveFile(
        params: SaveFileDialogParams(
          sourceFilePath: testFile.path,
          fileName: 'emergency_test.txt',
        ),
      );
      // Clean up
      ...
      print(result != null ? '✅ Emergency test SUCCESS' : '❌ Emergency test cancelled');
      return result != null;
    } catch (e) {
