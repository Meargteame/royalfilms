...
class _HomeScreenState extends State<HomeScreen> with SingleTickerProviderStateMixin {
  ...
  final List<String> _categories = ['Latest', 'Trending', 'Popular'];
  ...
  void _loadContent() {
    ...
    _mockService.popularContent.listen(
      (content) {
        if (mounted) {
          setState(() {
          ...
          : _errorMessage != null
              ? Center(
                  child: Column(
                    ...
                    children: [
                      const Icon(Icons.error_outline, color: AppTheme.errorColor, size: 50),
                    ...
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            final item = _popularContent[index];
                            return Padding(
                              padding: const EdgeInsets.symmetric(horizontal: 4),
                              child: ContentCard(
                                imageUrl: item['poster_url'] ?? 'https://via.placeholder.com/300x450',
                      ...
                      SizedBox(
                        ...
                        child: ListView.builder(
                          padding: const EdgeInsets.symmetric(horizontal: 12),
                          scrollDirection: Axis.horizontal,
                          itemCount: _latestContent.length,
                          itemBuilder: (context, index) {
                            final item = _latestContent[index];
                            return Padding(
                              padding: const EdgeInsets.symmetric(horizontal: 4),
                              child: ContentCard(
                                imageUrl: item['poster_url'] ?? 'https://via.placeholder.com/300x450',
                      ...
                      // Footer area with navigation tabs
                      const SizedBox(height: 40),
                      Container(
                        padding: const EdgeInsets.only(bottom: 8), // Added padding to avoid overflow
                        child: TabBar(
                      ...
                      SizedBox(
                        ...
                        child: TabBarView(
                          controller: _tabController,
                          children: [
                            _buildCategoryContentPage(_latestContent),
                            _buildCategoryContentPage(_trendingContent),
                            _buildCategoryContentPage(_popularContent),
                          ],
                        ),
                      ),
                    ],
  ...
  Widget _buildCategoryContentPage(List<dynamic> contentList) {
      ...
      child: Stack(
        children: [
          ...
          ClipRRect(
            ...
            child: Image.network(
              featuredItem['poster_url'] ?? 'https://via.placeholder.com/500x300',
              width: double.infinity,
              height: 240,
              fit: BoxFit.cover,
                      ...
                      ElevatedButton.icon(
                        onPressed: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (context) => DetailScreen(content: featuredItem),
                            ),
                          );
                        },
